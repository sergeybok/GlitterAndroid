...
public class GifImageView extends ImageView {
	...
	// Two methods that I added to integrate a self download upon creation of object
	// Sergey

	public static GifImageView newInstanceWithURL(Context c, String url) {
		GifImageView gif = new GifImageView(c);
		gif.setUrl(url);
		return gif;
	}
	...
	private synchronized static void pause(GifImageView view) {
		...
		if (info != null && !info.paused) {
			try { info.pause.acquire(); } catch (InterruptedException ex) {}
			info.paused = true;
		}
	}
	...
	private static void backgroundThread() {
		...
		try {
			...
			GifUtils.logWarning(TAG, ex);
		} catch (InterruptedException ex) {
			GifUtils.logError(TAG, ex, null);
		} finally {
